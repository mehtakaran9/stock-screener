...
| `RENDER_URL` | `https://your-service.onrender.com` — enables the **Render Keepalive** workflow, which pings the backend at 10:50 AM ET on weekdays so it is warm before the first scan at 11 AM. Note: GitHub disables scheduled workflows after 60 days of repository inactivity; push a commit or manually trigger the workflow to re-enable it. |
